...
describe("initProject", () => {
  test("creates a usable context setup and is idempotent", async () => {
    await withTempRepo(async (repo) => {
      ...
      expect(cursor).toContain('bun run barry -- fact draft --route "<route>" --prefix "<PREFIX>" ...');
      expect(cursor).toContain("`fact draft` is an authoring guardrail, not broad canonical CRUD");
